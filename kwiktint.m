...
%
% kwiktint(Name,Value,...) process with Name-Value pairs used to control aspects of the cluster cutting
%
% INPUT
%
...
% get some session data
    ...
    [h,~] = get_axona_headers([pwd '\' nme '.set']); % open first .set file to find dead channels - these shouldn't change

    pattern = 'saveEEG_ch_\d+';
    headers = {h.header{:}}';
    matches = cellfun(@(s) ~isempty(regexp(s, pattern, 'once')), headers);
    eeg_headers = headers(matches);
    eeg_values = logical(cell2mat(h.value(matches)));
    nums = cellfun(@(s) str2double(regexp(s, '\d+', 'match', 'once')), eeg_headers);
    eegch = nums(eeg_values);
    
    pattern = 'mode_ch_\d+';
    headers = {h.header{:}}';
    matches = cellfun(@(s) ~isempty(regexp(s, pattern, 'once')), headers);
    eeg_headers = headers(matches);
    eeg_values = cell2mat(h.value(matches));
    nums = cellfun(@(s) str2double(regexp(s, '\d+', 'match', 'once')), eeg_headers);
    grdch = nums(eeg_values==6);
    
    dedch = unique([eegch; grdch]);
...
%%%%%%%%%%%%%%%% Final Messages
    analysis_log({'kwiktint'},1,'version',{'v4.0.1'});
    fclose('all');
    toc1 = toc/60;
    disp(sprintf('Kwiktint has finished. It took %0.3f s (%0.3f mins)',toc,toc1)) % Stop counting time and display results
